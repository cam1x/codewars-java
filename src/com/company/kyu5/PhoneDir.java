...
public class PhoneDir {
    ...
    public static  String phone(String strng, String num) {
        ...
        List<String> notes= Arrays.asList(strng.split("\n"));
        ...
        for (String note:notes){
            ...
            if (phoneMatcher.find() && nameMatcher.find() && phoneMatcher.group().equals("+"+num)) {
                if (sb.toString().isEmpty()) {
                    sb.append("Phone => " + num + ", Name => " + nameMatcher.group().replaceAll("<|>","") +
                            ", Address => " + note.replace(phoneMatcher.group(), "").replace(nameMatcher.group(), "").
                            replaceAll("[^\\w.\\-\\s]|\\_", " ").trim() + "\n");
                }else{
                    return "Error => Too many people: "+num;
                }
            }
        }
        return (sb.toString().isEmpty())?"Error => Not found: "+num:sb.deleteCharAt(sb.length()-1).toString().replaceAll("\\s{2,}"," ");
    }
